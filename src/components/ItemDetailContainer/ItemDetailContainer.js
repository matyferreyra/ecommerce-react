...
const ItemDetailContainer = ()=> {
    const {id} = useParams(); ////acá estamos desestructurando un objeto y capturandolo (definiendo) en la variable "id" como una ruta dinámica

    const [item, setItem] = useState({}); //items seria una variable, y setItems el método por el cual se actualiza la variable
    
    const getData = (id) => {
        return new Promise ((resolve) => { //EL SEGMENTO DE LINEA 9 A 12 EMULA UNA PETICIÓN A UNA API, QUE DEMORA 2SEG EN TRAER LOS DATOS
